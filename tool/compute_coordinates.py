        ...
        if(nA != 0 and nB != 0):
            ...
            for i in range(nA):
                for j in range(nB):
                    ...
                    criterion1 = len(np.nonzero(score_midpts > th2)[
...
def get_coords(oriImg, model):
    ...
    for m in range(len(multiplier)):
        ...
        paf = st.resize(output1[0], orig_shape[:2],
                        preserve_range=True, order=1)
        heatmap_avg += heatmap
        paf_avg += paf
...
if __name__ == '__main__':
    ...
    for image_name in tqdm(img_list):
        ...
        oriImg = imread(os.path.join(input_folder, image_name))[
            :, :, ::-1]  # B,G,R order

        pose_cords = get_coords(oriImg, model)

        print("%s: %s: %s" % (image_name, str(list(pose_cords[:, 0])), str(
            list(pose_cords[:, 1]))), file=result_file)
        result_file.flush()
