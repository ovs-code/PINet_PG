...
threshold = 0.1
boxsize = 368
# scale_search = [0.5, 1, 1.5, 2]  # I dont know what this should do
scale_search = [1]
...
for image_name in tqdm(img_list):
    ...
    oriImg = st.resize(oriImg, (256,256), preserve_range=True, order=1)  # manually resizing the images to get the script running

    multiplier = [x * boxsize / oriImg.shape[0] for x in scale_search]

    heatmap_avg = np.zeros((orig_shape[0], orig_shape[1], 19))
    paf_avg = np.zeros((orig_shape[0], orig_shape[1], 38))
    ...
    for m in range(len(multiplier)):
        ...
        heatmap = st.resize(output2[0], orig_shape[:2], preserve_range=True, order=1)
        paf = st.resize(output1[0], orig_shape[:2], preserve_range=True, order=1)
        heatmap_avg += heatmap
        paf_avg += paf
